...
# One hostname, one A record. Same layout as the LAN address: / = Open WebUI, /portal /login /account = portal,
# /hub = admin, /grafana = Grafana, /status = public status, /v1 = API. No subdomains.
{hostname} {{
    tls {{
...
ACTIONS = {"/hub/api/ops": act_ops, "/hub/api/audit/clear": act_clear_log, "/hub/api/evidence/handoff": act_evidence_handoff, "/hub/api/policy": act_policy, "/hub/api/alerts": act_alerts, "/hub/api/models/pull": act_pull,
           ...
           "/hub/api/devices/issue": act_device_issue, "/hub/api/devices/revoke": act_device_revoke, "/hub/api/devices/delete": act_device_delete, "/hub/api/devices/require": act_device_require, "/hub/api/hostname": act_hostname}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _login_ok(self, user: str, ttl: str | None = None):
        ...
        return self._redirect("/portal", cookie=self._cookie(tok, age))
