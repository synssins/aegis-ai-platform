...
CHATPOOL_NARROW = os.environ.get("CHATPOOL_NARROW", "http://ollama-intel:11434")       # chat pool on one Arc (images keep the other)
CHATPOOL_WIDE = os.environ.get("CHATPOOL_WIDE", "http://ollama-intel-wide:11434")     # chat pool on both Arcs (images paused)
...
SERVICES = {  # name -> (purpose, network, protected)
    ...
    "litellm-db": ("Key/model store for LiteLLM", "backend", False), "ollama": ("Inference engine, NVIDIA pool (chat models, image classifier)", "backend", False),
    "ollama-intel": ("Inference engine, Intel Arc pool (safety classifier)", "backend", False),
    ...
    "fish-speech": ("Text-to-speech (apps profile)", "apps", False), "comfyui": ("Image generation on the NVIDIA pool (profile apps-nvidia) — behind the hub gate", "apps", False), "comfyui-intel": ("Image generation on an Intel Arc (profile apps) — behind the hub gate", "apps", False), "ollama-intel-wide": ("Chat pool on BOTH Arcs (wide mode; images paused) — switched by the hub", "backend", False),
}
...
NAV = [
    ...
    ("safety", "Safety", [("policy", "VetoGuard policy"), ("audit", "Audit log"), ("alerts", "Alerts"), ("galleries", "Galleries")]),
    ("models", "Models", [("installed", "Installed"), ("browse", "Browse"), ("store", "Image model store"), ("pull", "Pull"), ("exposed", "Exposed to apps")]),
...
def portal_models() -> list[dict]:
    """Models a portal user may pick: exposed through LiteLLM (admin: Models -> Exposed). Users never load or unload
    anything themselves: choosing a non-resident model asks the scheduler, which loads it under policy."""
    resident = set()
    for url in POOLS.values():
    ...
    for m in exposed_models():
        ...
        if not pub or not lm.startswith(("ollama/", "ollama_chat/")):
            continue
        base_url = (m.get("litellm_params") or {}).get("api_base", "")
        pool = next((k for k, v in POOLS.items() if v == base_url), "nvidia")
        out.append({"name": pub, "engine_model": base(lm), "resident": any(same(base(lm), r) for r in resident), "pool": pool, "protected": scheduler.protected(base(lm), policy())})
...
def p_installed(msg="", ok=True):
    exposed = {m.get("litellm_params", {}).get("model", "").replace("ollama/", "").replace("ollama_chat/", ""): m.get("model_name") for m in exposed_models()}
    pins = set(policy()["scheduler"].get("pinned", []))
    pool_sel = ('<select name="pool" title="GPU pool the model is served from; Ollama spreads a model over every card of the pool when it does not fit one">' + "".join(f'<option value="{k}">{k} pool</option>' for k in POOLS) + '</select> ') if len(POOLS) > 1 else ""
    ...
    for m in models:
        ...
        pin = "" if m["is_guard"] else (f'<form class="inline" method="post" action="/hub/api/models/pin">{csrf_field()}<input type="hidden" name="model" value="{esc(n)}">{"<input type=hidden name=unpin value=1>" if n in pins else ""}<button class="ghost" title="pinned models are never evicted by a user request">{"Unpin" if n in pins else "Pin"}</button></form>')
        ...
        rows += f'<tr><td>{esc(n)} {cap_icons(m.get("caps", []))}</td><td>{size:.1f} GiB</td><td>{"<span class=ok>resident</span>" if m["loaded"] else "<span class=mut>on disk</span>"}</td><td>{act}</td><td>{ld} {pin} {rm}</td></tr>'
    mode = chat_mode(); pins = set(policy()["scheduler"].get("pinned", []))
    modecard = f'''<div class="card"><h2 style="margin-top:0">Chat pool mode: <span class="{"warn" if mode == "wide" else "ok" if mode == "narrow" else "mut"}">{esc(mode)}</span></h2><p class="mut">Normal = one Arc for chat, the other for images. Wide = both Arcs for one large chat model; image generation is paused and users are told. Wide reverts to normal by itself after {policy()["scheduler"].get("wide_idle_min", 20)} idle minutes (Safety → VetoGuard policy). Switching takes about a minute (watchdog).</p>
<form class="inline" method="post" action="/hub/api/models/chatmode">{csrf_field()}<input type="hidden" name="mode" value="{"narrow" if mode == "wide" else "wide"}"><button class="{"ghost" if mode == "wide" else "danger"}">{"Back to normal (images on)" if mode == "wide" else "Go wide (images paused)"}</button></form></div>'''
    body = modecard + f'<div class="card">{mctl}<table><tr><th>Model</th><th>Size</th><th>State</th><th>Exposure</th><th></th></tr>{rows or "<tr><td colspan=5 class=mut>none</td></tr>"}</table></div><p class="mut">"Expose" registers the model in LiteLLM under a public name (through VetoGuard); models that advertise <b>tools</b> are registered on the native chat API so assistants such as Home Assistant get real tool calls — a model without <b>tools</b> can only answer in text. Guard models are never exposable; the active classifier is chosen in Safety → VetoGuard policy (or "Set as classifier" here — same action) and is loaded/unloaded by that choice, not by hand. Unload before removing.</p>'
...
def p_exposed(msg="", ok=True):
    ...
    return page("models", "exposed", "Exposed to apps", "Models LiteLLM currently serves — all through VetoGuard. The pool column is the GPU pool each public name is served from.", body, msg, ok)
...
PORTAL_SECTIONS = [  # (key, label, flag or None, kind, description)
    ...
    ("images", "Images", "images", "app", "Image generation (ComfyUI). Gated until its safety gate exists."),
    ("gallery", "Gallery", "images", "gallery", "Your generated images. Administrators may review galleries when review is enabled."),
...
def _section_target(key: str) -> str | None:
    dom = current_domain()
    return {"webui": "/", "images": "/comfy/", "grafana": "/grafana/", "status": "/status", "admin": "/hub/overview/dashboard", "settings": "/account"}.get(key)
...
def p_portal_section(key: str):
    ...
    if sec[3] == "placeholder":
        ...
        body = p_gallery_body(user, user, admin_view=False)
    else:
...
# ---------------------------------------------------------------- actions ------------------
def write_ops(req: dict) -> tuple[bool, str]:
    if os.path.exists(OPS_REQ):
        return False, "A watchdog request is already pending."
    req = {"id": secrets.token_hex(4), "ts": now(), "requested_by": getattr(REQ, "user", None) or "scheduler", **req}
    tmp = OPS_REQ + ".tmp"
    try:
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(req, f)
        ...
        return False, f"Could not write the request: {e}"
    return True, req["id"]


def chat_mode() -> str:
    return scheduler.mode(http, CHATPOOL_NARROW, CHATPOOL_WIDE)


def set_chat_mode(target: str, why: str) -> tuple[bool, str]:
    ok, rid = write_ops({"action": "sequence", "steps": scheduler.mode_steps(target)})
    audit("chat_mode_requested" if ok else "chat_mode_request_failed", target=target, why=why, id=rid)
    return ok, rid


def act_chat_mode(form):
    target = form.get("mode", "")
    if target not in ("narrow", "wide"):
        return p_installed("Unknown mode.", False)
    ok, rid = set_chat_mode(target, "administrator")
    return p_installed((f"Switching the chat pool to {'WIDE (both Arcs, images paused)' if target == 'wide' else 'normal (one Arc, images back)'} — request {rid}; takes about a minute." if ok else rid), ok)


def act_pin(form):
    m = form.get("model", ""); pol = policy(); pins = set(pol["scheduler"].get("pinned", []))
    ...
    if form.get("unpin"):
        ...
        pins.add(m)
    pol["scheduler"]["pinned"] = sorted(pins); save_json(POLICY_FILE, pol); audit("model_pin", model=m, pinned=m in pins)
    return p_installed(f"{m} {'pinned: users cannot evict it' if m in pins else 'unpinned'}.")


def wide_watch() -> None:
    """Revert wide chat mode after idle (policy wide_idle_min). Runs every minute in the hub."""
    since = time.time()
    while True:
        time.sleep(60)
        try:
            if chat_mode() == "wide":
                idle_min = policy()["scheduler"].get("wide_idle_min", 20)
                if time.time() - max(since, scheduler.last_chat_use()) > idle_min * 60 and not os.path.exists(OPS_REQ):
                    set_chat_mode("narrow", f"idle {idle_min} min")
            else:
                since = time.time()
        except Exception as e:  # noqa: BLE001
            print(f"wide_watch: {e}", file=sys.stderr)
...
def act_policy(form):
    ...
    im = form.get("image_model", pol["images"]["classifier_model"]).strip()
    if im and (not MODEL_RE.match(im) or "vision" not in model_caps(im)):
    ...
    try:
        ...
        return p_policy("Scheduler minutes must be integers.", False)
    if sc != pol["scheduler"]:
...
# ================================================================ image gate (ComfyUI) ===========
GATE_JOBS: dict[str, dict] = {}          # prompt_id -> job (user, texts, files, results per output filename)
...
def veto_audit_image(user: str, reason_codes: list[str], detail: str, action: str, model: str, prompt_id: str, ip: str, fp: str, evidence_id: str | None, immutable: bool) -> None:
    ...
    try:
        ...
        print(f"veto audit write failed: {e}", file=sys.stderr)


def gpu_cards() -> list[dict]:
    """Every card the platform can use, per pool: NVIDIA live from DCGM, Intel declared (free ≈ total − resident on that pool).
    Fit badges and the status page use this; it is guidance, never a control."""
    cards = []
    try:
        for i, g in sorted(metrics_snapshot().get("gpus", {}).items()):
            total = (g.get("mem_used") or 0) + (g.get("mem_free") or 0)
...
def image_classifier() -> tuple[str, str | None]:
    """(model, pool url where it is resident) — None url = not resident = generation refused."""
    m = policy()["images"].get("classifier_model", "")
    for pool, url in POOLS.items():
        st, ps = http("GET", url + "/api/ps", timeout=5)
...
def gate_text(user: str, texts: list[str]) -> tuple[bool, str]:
    ...
    models = [m["name"] for m in portal_models() if m["resident"]]
    if not models:
        return False, "no text model is loaded, so the prompt gate cannot run"
    st, j = http("POST", LITELLM + "/v1/chat/completions", {"model": models[0], "max_tokens": 1, "messages": [{"role": "user", "content": text}],
                 "metadata": {"aegis_surface": "image_prompt"}}, {"Authorization": f"Bearer {key}"}, timeout=120)
    if st == 200:
        return True, ""
    if st == 400 and "veto" in json.dumps(j).lower():
        return False, "Refused by the safety gate. This request has been logged."
    return False, f"the safety gate is unavailable ({st}); generation refused"


def gate_worker(pid: str) -> None:
    """Poll ComfyUI for the prompt's outputs, classify each, keep or destroy. Runs in a thread per prompt."""
    job = GATE_JOBS[pid]; t0 = time.time(); model, url = image_classifier()
    while time.time() - t0 < 1800:
        st, h = http("GET", f"{COMFY}/history/{pid}", timeout=15)
        rec = h.get(pid) if isinstance(h, dict) else None
        if rec and rec.get("status", {}).get("completed") or rec and rec.get("status", {}).get("status_str") == "error":
            break
        time.sleep(2)
    else:
        with GATE_LOCK: job["status"] = "timeout"
        return
    outputs = [im for o in (rec.get("outputs") or {}).values() for im in (o.get("images") or [])]
    for im in outputs:
        fn, sub, typ = im.get("filename", ""), im.get("subfolder", ""), im.get("type", "output")
        src = os.path.normpath(os.path.join(imagegate.STORE, typ, sub, fn))
        if not src.startswith(os.path.join(imagegate.STORE, typ)) or not os.path.isfile(src):
            continue
        if not url:
            res = {"ok": False, "verdict": None, "raw": "", "error": "image classifier not resident", "seconds": 0}
        else:
            res = imagegate.classify_image(src, model, url)
        action, why, codes = imagegate.decide(res, job["nsfw_ok"])
        entry = {"action": action, "why": why, "codes": codes, "seconds": res["seconds"], "verdict": res["verdict"]}
        try:
            _gate_apply(job, pid, fn, src, res, action, why, codes, entry, model)
...
def _gate_apply(job, pid, fn, src, res, action, why, codes, entry, model):
        if action == "keep":
            gid = imagegate.gallery_add(job["user"], src, {"prompt_id": pid, "orig": fn, "files": job["files"], "prompt_texts": job["texts"][:6], "nsfw": bool(res["verdict"] and (res["verdict"]["nsfw"] or res["verdict"]["sexual_content"])), "classifier": model, "description": res["verdict"].get("description", "")})
            entry["gid"] = gid
...
def _combo_choices(spec):
    """(choices list, setter) for an object_info input spec in either shape, or (None, None)."""
    if isinstance(spec, list) and spec:
        if isinstance(spec[0], list):
            return spec[0], (lambda v: spec.__setitem__(0, v))
        if spec[0] == "COMBO" and len(spec) > 1 and isinstance(spec[1], dict) and isinstance(spec[1].get("options"), list):
            return spec[1]["options"], (lambda v: spec[1].__setitem__("options", v))
    return None, None


def _comfy_filter_object_info(obj, allowed_names: set[str] | None):
    """Drop model filenames the user may not use from every chooser (allowed_names None = admin, nothing dropped)."""
    if allowed_names is None or not isinstance(obj, dict):
        return obj
    inv = imagegate.inventory()
    for node in obj.values():
        for group in ("required", "optional"):
            for k, spec in ((node.get("input") or {}).get(group) or {}).items():
                choices, setter = _combo_choices(spec)
                if choices and any(isinstance(x, str) and os.path.basename(x) in inv for x in choices):
                    setter([x for x in choices if not (isinstance(x, str) and os.path.basename(x) in inv) or os.path.basename(x) in allowed_names])
    return obj


def _comfy_filter_inputs(obj: dict, user: str) -> dict:
    """LoadImage-style choosers list only this user's own uploads (plus ComfyUI's bundled example)."""
    mine = set(imagegate.uploads_of(user)) | {"example.png"}
    for node in obj.values():
        for group in ("required", "optional"):
            for k, spec in ((node.get("input") or {}).get(group) or {}).items():
                choices, setter = _combo_choices(spec)
                if choices and len(spec) > 1 and isinstance(spec[1], dict) and spec[1].get("image_upload"):
                    setter([x for x in choices if x in mine])
    return obj


def allowed_store_names(user: str) -> set[str] | None:
    rec = users().get(user, {}); f = flags_of(rec)
    if is_admin(user):
        return None
    a = imagegate.attrs()
    return {n for n in imagegate.inventory() if (a.get(n) or {}).get("nsfw") == "no" or ((a.get(n) or {}).get("nsfw") == "yes" and f.get("images_nsfw"))}


GALLERY_JS = r"""
(()=>{const CSRF="__CSRF__",BASE="__BASE__",$=s=>document.querySelector(s),esc=t=>String(t??"").replace(/[&<>"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));
let items=[];const sel=new Set();
async function load(){const r=await fetch(BASE+"/api/list",{headers:{"X-CSRF":CSRF}});const j=await r.json();items=j.items;sel.clear();render()}
function render(){const g=$("#g-grid");if(!items.length){g.innerHTML='<div class="mut" style="padding:30px">No images yet.</div>';$("#g-count").textContent="";return}
g.innerHTML=items.map(it=>`<label class="g-card${sel.has(it.id)?" on":""}"><input type="checkbox" data-id="${it.id}" ${sel.has(it.id)?"checked":""}><img loading="lazy" src="${BASE}/img/${it.id}" alt=""><div class="g-cap"><span class="mut">${esc((it.ts||"").slice(0,16).replace("T"," "))}</span>${it.nsfw?' <span class="tag bad">NSFW</span>':""}<div class="g-desc">${esc(it.description||"")}</div></div></label>`).join("");
$("#g-count").textContent=items.length+" images"+(sel.size?" · "+sel.size+" selected":"");}
$("#g-grid").addEventListener("change",e=>{const c=e.target;if(c.dataset.id){c.checked?sel.add(c.dataset.id):sel.delete(c.dataset.id);c.closest(".g-card").classList.toggle("on",c.checked);$("#g-count").textContent=items.length+" images"+(sel.size?" · "+sel.size+" selected":"")}});
$("#g-all").onclick=()=>{items.forEach(i=>sel.add(i.id));render()};$("#g-none").onclick=()=>{sel.clear();render()};
$("#g-del").onclick=async()=>{if(!sel.size)return;$("#g-del").disabled=true;const r=await fetch(BASE+"/api/delete",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF":CSRF},body:JSON.stringify({ids:[...sel]})});const j=await r.json();$("#g-msg").textContent=j.msg||"";$("#g-del").disabled=false;load()};
$("#g-grid").addEventListener("click",e=>{const im=e.target.closest("img");if(im){window.open(im.src,"_blank","noopener")}});
load();})();
"""
GALLERY_CSS = """
.g-bar{display:flex;gap:8px;align-items:center;flex-wrap:wrap;padding:12px 16px;border-bottom:1px solid var(--line)}.g-wrap{flex:1;display:flex;flex-direction:column;min-width:0;overflow:auto}
.g-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;padding:16px}.g-card{position:relative;background:var(--card);border:1px solid var(--line);border-radius:10px;overflow:hidden;cursor:pointer;display:block}.g-card.on{border-color:#e5e5e5}
.g-card input{position:absolute;left:8px;top:8px;width:18px;height:18px;z-index:2}.g-card img{width:100%;aspect-ratio:1;object-fit:cover;display:block}.g-cap{padding:8px 10px;font-size:12px}.g-desc{color:#bbb;margin-top:2px}
"""


def p_gallery_body(user: str, owner: str, admin_view: bool) -> str:
    pol = policy()["images"]; base = f"/hub/gallery/{esc(owner)}" if admin_view else "/gallery"
    note = ("Administrator review of galleries is <b>ON</b>: administrators can open this gallery." if pol.get("gallery_review") else "Administrator review is <b>off</b>: only you can see this gallery.")
    return f"""<div class="g-wrap"><div class="g-bar"><b>{"Gallery of " + esc(owner) if admin_view else "Your gallery"}</b><span id="g-count" class="mut"></span><span style="margin-left:auto"></span>
<button id="g-all" class="ghost">Select all</button><button id="g-none" class="ghost">None</button><button id="g-del" class="danger">Delete selected</button><span id="g-msg" class="mut"></span></div>
<div class="mut" style="padding:8px 16px">{note} Deleting removes the image file, its record and ComfyUI's history entry — there is no other copy. Images that failed the safety gate never reached here.</div>
<div id="g-grid" class="g-grid"></div><style>{GALLERY_CSS}</style><script>{GALLERY_JS.replace("__CSRF__", CSRF).replace("__BASE__", base)}</script></div>"""


def p_galleries(msg="", ok=True):
    pol = policy()["images"]
    if not pol.get("gallery_review"):
        return page("safety", "galleries", "Galleries", "Administrator review of user galleries is off.", '<div class="card"><p>Turn on <b>Administrator review of user galleries</b> in Safety → VetoGuard policy to open galleries here. Users are told on their Gallery page whenever it is on.</p></div>', msg, ok)
    rows = "".join(f'<tr><td><a href="/hub/gallery/{esc(u)}/">{esc(u)}</a></td><td>{len(imagegate.gallery_list(u))}</td></tr>' for u in imagegate.gallery_users()) or '<tr><td colspan="2" class="mut">no galleries yet</td></tr>'
    return page("safety", "galleries", "Galleries", "Review is ON — every open is audited.", f'<div class="card"><table><tr><th>User</th><th>Images</th></tr>{rows}</table></div>', msg, ok)


def p_store(msg="", ok=True):
    rows = "".join(f'<tr><td><code>{esc(x["path"])}</code></td><td>{esc(browse._fmt_size(x["size"]))}</td><td class="{ {"yes": "bad", "no": "ok"}.get(x["nsfw"], "warn") }">{esc(x["nsfw"])}</td><td class="mut">{esc(x["source"])}</td><td><form class="inline" method="post" action="/hub/api/store/attr">{csrf_field()}<input type="hidden" name="name" value="{esc(x["name"])}"><select name="nsfw"><option value="no" {"selected" if x["nsfw"] == "no" else ""}>SFW</option><option value="yes" {"selected" if x["nsfw"] == "yes" else ""}>NSFW</option><option value="unclassified" {"selected" if x["nsfw"] == "unclassified" else ""}>unclassified</option></select> <button class="ghost">Set</button></form></td></tr>' for x in imagegate.store_listing()) or '<tr><td colspan="5" class="mut">no model files in comfyui/</td></tr>'
    return page("models", "store", "Image model store", "Per-file NSFW attribute. Unclassified files are usable by administrators only; NSFW files need the user's NSFW grant. Set automatically by the browser from the registry's flag; hand-copied files start unclassified.", f'<div class="card"><table><tr><th>File</th><th>Size</th><th>NSFW</th><th>Source</th><th></th></tr>{rows}</table></div>', msg, ok)


def act_store_attr(form):
    name, v = form.get("name", ""), form.get("nsfw", "")
    if name not in imagegate.inventory() or v not in ("yes", "no", "unclassified"):
        return p_store("Unknown file or value.", False)
    imagegate.set_attr(name, v, getattr(REQ, "user", "admin")); audit("store_attr_set", file=name, nsfw=v)
    return p_store(f"{name}: {v}.")
...
def act_browse_download(form) -> dict:
    ...
    if jid:
        imagegate.set_attr(os.path.basename(form.get("name", "")), "yes" if form.get("nsfw") == "1" else "no", f"browser:{src}", source=f"{src}:{form.get('model_id', '')}", sha256=sha)
...
$("#br-mbody").querySelectorAll(".br-dl").forEach(b=>b.onclick=async()=>{b.disabled=true;const j=await post("/hub/api/browse/download",{src:d.src,url:b.dataset.url,name:b.dataset.name,sha256:b.dataset.sha,size:b.dataset.size,subdir:b.dataset.subdir,nsfw:b.dataset.nsfw,model_id:b.dataset.model});$("#br-msg").textContent=j.msg;$("#br-msg").className=j.ok?"ok":"bad";jobs()});
...
def act_expose(form):
    ...
    prov = provider_for(m); pool = form.get("pool", "nvidia") if form.get("pool", "nvidia") in POOLS else "nvidia"
    st, j = litellm("POST", "/model/new", {"model_name": pub, "litellm_params": {"model": f"{prov}/{m}", "api_base": POOLS[pool]}}); ok = st == 200
    audit("model_exposed" if ok else "model_expose_failed", model=m, public=pub, provider=prov, pool=pool, tools="tools" in model_caps(m), status=st)
...
ACTIONS = {"/hub/api/ops": act_ops, "/hub/api/audit/clear": act_clear_log, "/hub/api/evidence/handoff": act_evidence_handoff, "/hub/api/policy": act_policy, "/hub/api/alerts": act_alerts, "/hub/api/models/pull": act_pull,
           ...
           "/hub/api/browse/settings": act_browse_settings, "/hub/api/store/attr": act_store_attr, "/hub/api/models/chatmode": act_chat_mode, "/hub/api/models/pin": act_pin, "/hub/api/devices/issue": act_device_issue, "/hub/api/devices/revoke": act_device_revoke, "/hub/api/devices/delete": act_device_delete, "/hub/api/devices/require": act_device_require, "/hub/api/hostname": act_hostname}
PAGES = {("overview", "dashboard"): p_dashboard, ("overview", "metrics"): p_metrics, ("overview", "services"): p_services, ("safety", "policy"): p_policy,
         ("safety", "audit"): p_audit, ("safety", "alerts"): p_alerts, ("safety", "galleries"): p_galleries, ("models", "store"): p_store, ("models", "installed"): p_installed,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _comfy_upload(self, user: str, raw: bytes):
        ...
        if "multipart/form-data" not in ctype:
            return self._send(400, '{"error":"multipart expected"}', "application/json")
        ...
        for part in msg.iter_parts():
            if part.get_param("name", header="content-disposition") == "image":
                data, name = part.get_payload(decode=True), part.get_filename() or "upload.png"
        if not data:
            return self._send(400, '{"error":"no image field"}', "application/json")
        kind = "png" if data[:8] == b"\x89PNG\r\n\x1a\n" else "jpg" if data[:3] == b"\xff\xd8\xff" else "webp" if data[:4] == b"RIFF" and data[8:12] == b"WEBP" else None
        if not kind or len(data) > 25 * 2**20:
            return self._send(415, '{"error":"PNG, JPEG or WebP up to 25 MB"}', "application/json")
        f = flags_of(users().get(user, {})); ip = client_ip(self); fp = self._fp()
        model, url = image_classifier()
        if not url:
            return self._send(503, json.dumps({"error": f"The image safety classifier ({model}) is not loaded; uploads are refused."}), "application/json")
        tmp = os.path.join(imagegate.STORE, "temp", f".upload-{secrets.token_hex(6)}.{kind}")
        ...
        safe = re.sub(r"[^A-Za-z0-9._-]", "_", os.path.basename(name))[:60]
        final = f"{user}-{secrets.token_hex(3)}-{safe}"
        final = final if final.lower().endswith(("." + kind, ".jpeg")) else final + "." + kind
        os.replace(tmp, os.path.join(imagegate.STORE, "input", final)); os.chmod(os.path.join(imagegate.STORE, "input", final), 0o644)
        imagegate.uploads_add(user, final)
        audit("upload_kept", user=user, file=final, classifier=model, seconds=res["seconds"])
        return self._send(200, json.dumps({"name": final, "subfolder": "", "type": "input"}), "application/json")
    ...
    def _comfy_prompt(self, user: str, raw: bytes):
        try:
            ...
            return self._send(400, json.dumps({"error": "bad workflow", "node_errors": {}}), "application/json")
        rec = users().get(user, {}); f = flags_of(rec); ip = client_ip(self); fp = self._fp()
        if chat_mode() != "narrow":
            audit("image_prompt_refused", user=user, why="chat pool in wide mode (images paused)")
            return self._send(503, json.dumps({"error": "Image generation is paused: an administrator gave both accelerators to chat (wide mode). It returns automatically when chat goes idle, or when an administrator switches back.", "node_errors": {}}), "application/json")
        model, url = image_classifier()
        if not url:
            audit("image_prompt_refused", user=user, why=f"image classifier {model} not resident")
            return self._send(503, json.dumps({"error": f"The image safety classifier ({model}) is not loaded; generation is refused until an administrator loads it.", "node_errors": {}}), "application/json")
        files = imagegate.file_refs(wf)
        foreign = [n for n in imagegate.input_refs(wf) if imagegate.uploads_owner(n) != user]
        if foreign:
            audit("image_prompt_refused", user=user, why="input image not uploaded by this account", files=",".join(foreign))
            return self._send(403, json.dumps({"error": "That input image was not uploaded by your account.", "node_errors": {}}), "application/json")
        ok, why = imagegate.files_allowed(files, bool(f.get("images_nsfw")), is_admin(user))
        if not ok:
        ...
        if not ok:
            audit("image_prompt_refused", user=user, why=why[:120], files=",".join(files), ip=ip, fingerprint=fp or "none")
            return self._send(400, json.dumps({"error": why, "node_errors": {}}), "application/json")
        st, j = http("POST", COMFY + "/prompt", {"prompt": wf, "client_id": body.get("client_id"), "extra_data": body.get("extra_data") or {}, "front": is_admin(user)}, timeout=60)
        if st != 200 or not isinstance(j, dict) or not j.get("prompt_id"):
            return self._send(st or 502, json.dumps(j) if isinstance(j, dict) else json.dumps({"error": str(j)[:200]}), "application/json")
        pid = j["prompt_id"]
        with GATE_LOCK:
            GATE_JOBS[pid] = {"user": user, "ts": time.time(), "texts": texts, "files": files, "nsfw_ok": bool(f.get("images_nsfw")), "ip": ip, "fp": fp, "status": "pending", "results": {}}
            for k in [k for k, v in GATE_JOBS.items() if time.time() - v["ts"] > 86400]:
                GATE_JOBS.pop(k, None)
        audit("image_prompt_accepted", user=user, prompt_id=pid, files=",".join(files), texts=len(texts), ip=ip, fingerprint=fp or "none")
        threading.Thread(target=gate_worker, args=(pid,), daemon=True).start()
        return self._send(200, json.dumps(j), "application/json")

    def _comfy_get(self, user: str, p: str):
        q = {k: (v[0] if v else "") for k, v in REQ.q.items()}
        f = flags_of(users().get(user, {}))
        if not f.get("images"):
            return self._send(403, "images are not granted", "text/plain")
        sub = p.replace("/comfy/api", "", 1) if p.startswith("/comfy/api") else p.replace("/comfy", "", 1)
        if sub == "/view":
            if q.get("type", "output") == "input":
                fn = os.path.basename(q.get("filename", ""))
                if imagegate.uploads_owner(fn) != user or not os.path.isfile(os.path.join(imagegate.STORE, "input", fn)):
                    return self._send(404, "no such input image for this account", "text/plain")
                with open(os.path.join(imagegate.STORE, "input", fn), "rb") as fh:
                    data = fh.read()
                return self._send(200, data, "image/png" if fn.lower().endswith(".png") else "image/jpeg")
            if q.get("type", "output") != "output":
                return self._send(403, "previews and temp images are disabled by policy", "text/plain")
            fn = os.path.basename(q.get("filename", ""))
            for _ in range(50):                                   # long-poll until the gate has decided
                with GATE_LOCK:
                    hit = next(((pid, j) for pid, j in sorted(GATE_JOBS.items(), key=lambda kv: kv[1]["ts"], reverse=True) if j["user"] == user and fn in j["results"]), None)
                    pending = any(j["user"] == user and j["status"] == "pending" for j in GATE_JOBS.values())
                if hit or not pending:
                    break
                time.sleep(0.5)
            if not hit:
                return self._send(404, "no such image for this account", "text/plain")
            entry = hit[1]["results"][fn]
            if entry["action"] != "keep":
                return self._send(403, "This image was removed by the safety gate. " + ("This has been logged." if entry["action"] != "destroy_nsfw" else "Your account does not have the NSFW grant."), "text/plain")
            path = imagegate.gallery_path(user, entry["gid"])
            if not path:
                return self._send(404, "deleted", "text/plain")
            with open(path, "rb") as fh:
                data = fh.read()
            return self._send(200, data, "image/png" if path.endswith(".png") else "image/jpeg")
        if sub.startswith("/object_info"):
            st, j = http("GET", COMFY + sub, timeout=30)
            if isinstance(j, dict):
                j = _comfy_filter_object_info(j, allowed_store_names(user)); j = _comfy_filter_inputs(j, user)
            return self._send(st or 502, json.dumps(j) if isinstance(j, (dict, list)) else str(j), "application/json")
        if sub.startswith("/history"):
            st, j = http("GET", COMFY + sub + ("?" + urllib.parse.urlencode(q) if q else ""), timeout=30)
            if isinstance(j, dict):
                with GATE_LOCK:
                    mine = {pid for pid, jb in GATE_JOBS.items() if jb["user"] == user}
                j = {k: v for k, v in j.items() if k in mine}
            return self._send(st or 502, json.dumps(j), "application/json")
        return self._send(404, "unknown", "text/plain")

    def _gallery_owner(self, user: str, p: str) -> tuple[str | None, str]:
        """(owner, rest-of-path). Own gallery at /gallery/...; admin review at /hub/gallery/<owner>/... when policy allows."""
        if p.startswith("/gallery/"):
            return user, p[len("/gallery"):]
        m = re.match(r"^/hub/gallery/([a-z0-9._-]+)(/.*)?$", p)
        if m and is_admin(user) and policy()["images"].get("gallery_review"):
            return m.group(1), m.group(2) or "/"
        return None, ""

    def _gallery_get(self, user: str, p: str):
        owner, rest = self._gallery_owner(user, p)
        if not owner:
            return self._send(403, "not permitted", "text/plain")
        if owner != user and rest == "/":
            audit("gallery_reviewed", owner=owner)
        if rest == "/":
            return self._send(200, f'<!doctype html><html><head><meta charset="utf-8"><title>Gallery — {esc(owner)}</title><style>{CSS}</style></head><body style="display:flex;min-height:100vh">{p_gallery_body(user, owner, admin_view=(owner != user))}</body></html>')
        if rest == "/api/list":
            return self._send(200, json.dumps({"items": imagegate.gallery_list(owner)}), "application/json")
        m = re.match(r"^/img/([0-9TZ]+-[0-9a-f]{6})$", rest)
        if m:
            path = imagegate.gallery_path(owner, m.group(1))
            if not path:
    ...
    def _gallery_post(self, user: str, p: str, raw: bytes):
        owner, rest = self._gallery_owner(user, p)
        if not owner or rest != "/api/delete":
            return self._send(403, '{"error":"not permitted"}', "application/json")
        try:
            ids = [str(x) for x in json.loads(raw.decode()).get("ids", [])][:500]
        except (ValueError, UnicodeDecodeError, AttributeError):
            return self._send(400, '{"error":"bad request"}', "application/json")
        pids = imagegate.gallery_delete(owner, ids)
        if pids:
            http("POST", COMFY + "/history", {"delete": pids}, timeout=15)
        audit("gallery_deleted", owner=owner, count=len(ids), by=user)
        return self._send(200, json.dumps({"ok": True, "msg": f"Deleted {len(ids)} image(s) — file, record and generation history."}), "application/json")
    ...
    def _chat_stream(self, user: str, raw: bytes):
        ...
        pm = next((m for m in portal_models() if m["name"] == model), None)
        if not pm:
            return self._send(400, '{"error":"that model is not available"}', "application/json")
        ...
        if pm["pool"] != next((k for k, v in POOLS.items() if v == GUARD_OLLAMA), None):      # always ask: /api/ps lags evictions
            ok, why = scheduler.ensure_resident(http, POOLS[pm["pool"]], pm["engine_model"], is_admin(user), policy(), audit)
            if not ok:
        ...
        scheduler.note_use(pm["engine_model"], is_admin(user), False)
        try:
    ...
    def do_GET(self):
        ...
        if p == "/gallery" or p.startswith("/gallery/") or re.match(r"^/hub/gallery/[a-z0-9._-]+(/|$)", p):
            return self._gallery_get(user, p if p != "/gallery" else "/gallery/")
    ...
    def do_POST(self):
        ...
        if p in ("/comfy/upload/image", "/comfy/api/upload/image"):
            user = self._session()
            if not user:
                return self._send(401, '{"error":"signed out"}', "application/json")
            if self.headers.get("Sec-Fetch-Site", "same-origin") not in ("same-origin", "none"):
                return self._send(403, '{"error":"cross-site request refused"}', "application/json")
            REQ.user = user; rec = users().get(user, {})
            if rec.get("must_change") or getattr(REQ, "limited", False) or not flags_of(rec).get("images"):
                return self._send(403, '{"error":"images are not granted to this account"}', "application/json")
            return self._comfy_upload(user, self.rfile.read(min(n, 26 * 2**20)))
        if p in ("/comfy/prompt", "/comfy/api/prompt"):
            user = self._session()
            if not user:
                return self._send(401, '{"error":"signed out"}', "application/json")
            if self.headers.get("Sec-Fetch-Site", "same-origin") not in ("same-origin", "none"):
            ...
            if rec.get("must_change") or getattr(REQ, "limited", False) or not flags_of(rec).get("images"):
                return self._send(403, '{"error":"images are not granted to this account"}', "application/json")
            return self._comfy_prompt(user, self.rfile.read(min(n, 4 * 2**20)))
        if p.startswith("/gallery/api/") or re.match(r"^/hub/gallery/[a-z0-9._-]+/api/", p):
            ...
            if not user:
                return self._send(401, '{"error":"signed out"}', "application/json")
            REQ.user = user
            return self._gallery_post(user, p, self.rfile.read(min(n, 65536)))
