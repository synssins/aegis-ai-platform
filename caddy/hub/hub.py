...
SERVICES = {  # name -> (purpose, network, protected)
    ...
    "fish-speech": ("Text-to-speech (apps profile)", "apps", False), "comfyui": ("Image generation (apps profile) — reachable only through the hub gate", "apps", False),
}
...
def _comfy_filter_object_info(obj, allowed_names: set[str] | None):
    """Drop model filenames the user may not use from every list-typed input (allowed_names None = admin, nothing dropped)."""
    ...
    for node in obj.values():
        for group in ("required", "optional"):
            for k, spec in ((node.get("input") or {}).get(group) or {}).items():
                if isinstance(spec, list) and spec and isinstance(spec[0], list) and any(isinstance(x, str) and os.path.basename(x) in inv for x in spec[0]):
                    spec[0] = [x for x in spec[0] if not (isinstance(x, str) and os.path.basename(x) in inv) or os.path.basename(x) in allowed_names]
    return obj
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _comfy_get(self, user: str, p: str):
        ...
        if sub.startswith("/object_info"):
            ...
            return self._send(st or 502, json.dumps(_comfy_filter_object_info(j, allowed_store_names(user))) if isinstance(j, (dict, list)) else str(j), "application/json")
