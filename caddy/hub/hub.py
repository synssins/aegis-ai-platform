...
# apex = portal (+ admin, status, API); chat. = Open WebUI; grafana. = Grafana. DNS: A records for the apex, chat, grafana.
{hostname}, chat.{hostname}, grafana.{hostname} {{
    tls {{
    ...
    @chat host chat.{hostname}
    handle @chat {{
        header -X-Frame-Options
        header Content-Security-Policy "frame-ancestors https://{hostname}"
...
def page(section, sub, title, subtitle, body, msg="", ok=True):
    ...
    dom = current_domain()
    apps = (f'<a href="https://chat.{esc(dom)}/">Open WebUI ↗</a>' if dom else '<a href="/">Open WebUI ↗</a>') + ((f'<a href="https://grafana.{esc(dom)}/">Grafana ↗</a>' if dom else '') if GRAFANA_ENABLED else '')
...
def p_devices(msg="", ok=True, bundle=None):
    ...
    rows = "".join(f'<tr><td>{esc(v.get("name"))}</td><td><code>{esc(fp[:16])}…</code>{" <span class=\"tag ok\">this device</span>" if fp == fp_now else ""}</td><td class="mut">{esc((v.get("issued") or "")[:10])} → {esc((v.get("expires") or "")[:10])}</td><td class="mut">{esc((v.get("last_seen") or "never")[:19])}</td><td class="{"bad" if v.get("revoked") else "ok"}">{"revoked " + esc((v.get("revoked") or "")[:10]) if v.get("revoked") else "active"}</td><td>{"" if v.get("revoked") else f"<form class=inline method=post action=/hub/api/devices/revoke>{csrf_field()}<input type=hidden name=fp value={esc(fp)}><button class=danger>Revoke</button></form>"}</td></tr>' for fp, v in sorted(d.get("devices", {}).items(), key=lambda kv: kv[1].get("issued", ""), reverse=True)) or '<tr><td colspan="6" class="mut">no devices issued</td></tr>'
...
def p_users(msg="", ok=True, invite=None):
    ...
    banner = f'<div class="card"><b>Invite for {esc(invite["user"])} — shown once.</b><p>Give them the username and this code; they sign in at <code>/login</code> with the code as the password, then set their own password and enrol MFA.</p><div class="key">{esc(invite["code"])}</div></div>' if invite else ""
...
def _section_target(key: str) -> str | None:
    dom = current_domain()
    return {"chat": f"https://chat.{dom}/" if dom else None, "grafana": f"https://grafana.{dom}/" if dom else None, "status": "/status", "admin": "/hub/overview/dashboard", "settings": "/account"}.get(key)
...
def p_portal_section(key: str):
    ...
    if sec[3] == "placeholder":
        ...
        body = f'<div class="ph"><h1>{esc(sec[1])}</h1><p class="mut">Available once a public hostname is configured (Admin → Gateway → Public hostname). Until then, Chat is at <code>https://{esc(LAN_IP)}/</code>.</p></div>'
    else:
...
PAGES = {("overview", "dashboard"): p_dashboard, ("overview", "metrics"): p_metrics, ("overview", "services"): p_services, ("safety", "policy"): p_policy,
         ...
         ("models", "pull"): p_pull, ("models", "exposed"): p_exposed, ("access", "users"): p_users, ("access", "keys"): p_keys, ("access", "devices"): p_devices, ("access", "account"): p_account,
...
class Handler(BaseHTTPRequestHandler):
            ...
            if k == "aegis_hub":
                ...
                if d and d.get("kind") == "session":
                    ...
                    if rec and d.get("epoch", 0) == rec.get("session_epoch", 0):
                        ...
                        REQ.limited = bool(d.get("limited"))
                        return d["u"]
        return None
    ...
    def _login_ok(self, user: str):
        ...
        return self._redirect("/", cookie=self._cookie(tok, SESSION_TTL))
    ...
    def do_GET(self):
        ...
        if p.startswith("/hub") and not is_admin(user):
            audit("admin_refused", user=user, path=p); return self._send(403, plain_page("Admin only", '<p>This area is for administrators. <a href="/">Back to the portal</a>.</p>'))
        if p == "/hub":
    ...
    def do_POST(self):
        ...
        if p in ("/hub/login", "/login"):
            ...
            if rec.get("must_change"):
                REQ.user = user
                tok = sign({"kind": "session", "u": user, "epoch": rec.get("session_epoch", 0), "exp": time.time() + 900, "n": secrets.token_hex(8), "fp": self._fp(), "limited": True})
        ...
        if p == "/hub/account/password":
            ...
            if out is None:
                ...
                if not rec.get("totp"):
                    sec = base64.b32encode(secrets.token_bytes(20)).decode().rstrip("=")
                    return self._send(200, p_enrol(self._pre(user, "enrol"), sec, user), cookie=self._cookie("", 0))
                return self._login_ok(user)   # forced change completed -> new epoch session
            rec = users().get(user, {}); tok = sign({"kind": "session", "u": user, "epoch": rec.get("session_epoch", 0), "exp": time.time() + SESSION_TTL, "n": secrets.token_hex(8), "fp": self._fp()})
