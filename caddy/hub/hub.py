...
DEFAULT_POLICY = {"guard": {"model": "llama-guard3:1b", "chunk_chars": 6000, "chunk_overlap": 600, "max_chunks": 100, "timeout": 60, "concurrency": 2},
                  ...
                  "images": {"classifier_model": "gemma3:27b", "gallery_review": False}, "updated": None, "updated_by": None}
...
SERVICES = {  # name -> (purpose, network, protected)
    ...
    "fish-speech": ("Text-to-speech (apps profile)", "apps", False), "comfyui": ("Image generation on the NVIDIA pool (profile apps-nvidia) — behind the hub gate", "apps", False), "comfyui-intel": ("Image generation on an Intel Arc (profile apps) — behind the hub gate", "apps", False),
}
...
ACTIONS = {"/hub/api/ops": act_ops, "/hub/api/audit/clear": act_clear_log, "/hub/api/evidence/handoff": act_evidence_handoff, "/hub/api/policy": act_policy, "/hub/api/alerts": act_alerts, "/hub/api/models/pull": act_pull,
           ...
           "/hub/api/browse/settings": act_browse_settings, "/hub/api/store/attr": act_store_attr, "/hub/api/devices/issue": act_device_issue, "/hub/api/devices/revoke": act_device_revoke, "/hub/api/devices/delete": act_device_delete, "/hub/api/devices/require": act_device_require, "/hub/api/hostname": act_hostname}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _comfy_prompt(self, user: str, raw: bytes):
        ...
        st, j = http("POST", COMFY + "/prompt", {"prompt": wf, "client_id": body.get("client_id"), "extra_data": body.get("extra_data") or {}}, timeout=60)
