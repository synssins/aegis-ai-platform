...
{hostname}, chat.{hostname}, grafana.{hostname} {{
    ...
    @sub host chat.{hostname} grafana.{hostname}
    header @sub Content-Security-Policy "frame-ancestors https://{hostname}"
    @apex host {hostname}
    header @apex Content-Security-Policy "frame-ancestors 'self'"
    handle @chat {{
    ...
    def do_POST(self):
        ...
        if p in ("/hub/login/totp", "/hub/login/enroll"):
            ...
            if p == "/hub/login/enroll" and d.get("stage") == "enroll":
                ...
                if c is None:
                    ...
                    return self._send(401, p_enroll(self._pre(user, "enroll"), sec, user, "Code did not match — check the clock on your device and try again.", False))
        ...
        if p == "/hub/account/password":
            ...
            if out is None:
                ...
                if not rec.get("totp"):
                    sec = base64.b32encode(secrets.token_bytes(20)).decode().rstrip("=")
                    return self._send(200, p_enroll(self._pre(user, "enroll"), sec, user), cookie=self._cookie("", 0))
