...
- **Device identity implemented:** hub-managed device CA (P-256, key Fernet-encrypted), Access → Devices (issue PKCS#12 bundles once, revoke, require-for-admin), Caddy `client_auth mode request` on every site with client-supplied `X-Device-*` headers stripped and the TLS fingerprint forwarded to hub and LiteLLM; sessions bound to the fingerprint; fingerprints in login audit, veto audit and evidence. Tests 5.19–5.21.
