...
11c. Hub admin: on first start the hub prints a one-time bootstrap password (`docker logs hub`); sign in, enrol MFA, set your password. `HUB_SECRET_KEY` must be set in `.env` first.
